...
<?php
  ...
  function addedForm($title,$dateStart,$dateFinish,$description,$studentsfor,$place){
    echo '<form action="addingNewVacancy.php" method="post">
      <div class="grid-x" style="min-height: 500px; padding-top:20px; padding-bottom: 20px;">
        <div class="small-10 medium-offset-1 medium-10 medium-offset-1 large-10 large-offset-1 cell">
          ...
          <div style="text-align: center;"><input type="submit" class="registration-btn" value="Добавить"></div>
        </div>
      </div>
    </form>';
  }
if (empty($_SESSION['category'])){
  ...
  //если пост не пустой, то то разбираем его выводим сообщение
    ...
    if ($_POST['title'] <> "" && $_POST['dateStart'] <> "" && $_POST['dateFinish'] <> "" && $_POST['description'] <> "" && $_POST['studentsfor'] <> "" && $_POST['place'] <> "") {
      ...
      $sql = "INSERT INTO `vacancies`(`title`, `dateStart`, `dateFinish`, `description`, `studentsfor`, `place`, `dateAdd`, `id_employers`, `userAddId`)
            VALUES (?,?,?,?,?,?,?,?,?)";
      $result = executeRequest($pdo,$sql,[$_POST['title'],$_POST['dateStart'],$_POST['dateFinish'],$_POST['description'],$_POST['studentsfor'],$_POST['place'],$date,$idEmployers,$idUser]);
      printValue("Вакансия успешно добавлена!<br>Добавить еще?");
      addedForm("","","","","","");
    } else {
