    ...
    <div class="grid-x link-block">
      ...
      <div class="small-10 large-10 columns">
        ...
        <form action="" method="post">
          <input type="radio" name="pokemon" value="Red" id="pokemonRed"><label for="pokemonRed">Студент</label>
          <br>
          <input type="radio" name="pokemon" value="Blue" id="pokemonBlue"><label for="pokemonBlue">Работодатель</label>
          <br>
		  <input type="submit" class="button" value="Подтвердить"></input>
        </form>	  
        <?php 
        if (!empty($_POST)){
          if ($_POST["pokemon"] == "Red"){
            exit("<html><head><meta http-equiv='Refresh' content='0; URL=../registrationStident.php'></head></html>");
          } else if ($_POST["pokemon"] == "Blue") {
